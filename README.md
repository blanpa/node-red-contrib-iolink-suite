...
[![npm](https://img.shields.io/npm/v/node-red-contrib-iolink-suite?logo=npm&label=npm)](https://www.npmjs.com/package/node-red-contrib-iolink-suite)
[![tests](https://github.com/blanpa/node-red-contrib-iolink-suite/actions/workflows/test.yml/badge.svg)](https://github.com/blanpa/node-red-contrib-iolink-suite/actions/workflows/test.yml)
[![Node-RED ≥ 3.0](https://img.shields.io/badge/Node--RED-%E2%89%A5%203.0-8f0000?logo=nodered&logoColor=white)](https://nodered.org)
[![Node ≥ 18](https://img.shields.io/badge/node-%E2%89%A5%2018-339933?logo=node.js&logoColor=white)](https://nodejs.org)
[![licence: Apache-2.0](https://img.shields.io/github/license/blanpa/node-red-contrib-iolink-suite)](LICENSE)
...
The six master-bound nodes share one config node, which talks to the master
through a profile and decodes through the IODD library. `iodd decode` uses the
same library with no master at all:
...
flowchart LR
  ...
  master["iolink master<br/>config node: connection,<br/>IODD store, identity cache"]
  read & write & param & scan & event --> master
  subgraph adapters["lib/adapters, raw bytes only"]
    ifm["ifm IoT Core"]
  ...
  iodd["lib/iodd<br/>IODD parser and codec"]
...
```mermaid
flowchart TD
  ids["vendorId and deviceId<br/>from the port, or pinned on the node"] --> mem{"parsed in memory?"}
  mem -- yes --> use["decode"]
  mem -- no --> failed{"not found less than<br/>retry after ago?"}
  failed -- yes --> err["error, no request sent"]
  failed -- no --> folder{"IODD folder holds it?"}
  folder -- yes --> use
  folder -- no --> cache{"disk cache holds it?"}
  cache -- yes --> use
  cache -- no --> offline{"offline?"}
  offline -- yes --> remember["remembered as missing<br/>until retry after"] --> err
  offline -- no --> finder["download from IODDfinder"] --> found{"found?"}
  found -- yes --> use
  found -- no --> remember
```
...
sequenceDiagram
  ...
  participant R as iolink read
  participant C as identity cache
  participant M as IO-Link master
  participant S as IODD store
  F->>R: msg, port 1
  R->>C: who is on port 1?
  alt answer younger than re-check
    C-->>R: vendorId, deviceId, product, serial
  else expired, or a shorter re-check asked
    C->>M: port status, identification
    M-->>C: vendorId 999, deviceId 4242
    C-->>R: kept for the next re-check
  end
  R->>S: IODD for 999 / 4242
  S-->>R: parsed device
  R->>M: process data in, port 1
  M-->>R: 092B0929
  R->>R: decode through the IODD
  R-->>F: payload, meta, device, iolink
```
...
A port as the node sees it, with `event.direction` on the transitions. A master
that stops answering is its own state rather than every device leaving, and
the last known state is carried through it:
...
stateDiagram-v2
  ...
  down: master unreachable, last state kept
  [*] --> empty: baseline, silent
  [*] --> connected: baseline, silent
  empty --> connected: appeared
  connected --> empty: disappeared
  connected --> connected: device changed, status changed
  empty --> down: unreachable
  connected --> down: unreachable
  down --> connected: reachable
  down --> empty: reachable, or disappeared
```
...
publishes to npm, and creates a GitHub release with that version's changelog
section as its notes.

It publishes through npm's **trusted publishing**: there is no token in the
repository, the workflow identifies itself with a GitHub OIDC token, and npm
attaches provenance naming the commit and the run. Two things have to be in
place once:

1. **The package has to exist on npm**, because a trusted publisher is
   configured on a package's settings page. npm cannot yet do a first publish
   this way, so the very first version goes up by hand, as a placeholder
   below the first real release:

   ```bash
   docker run -it --rm -v "$PWD":/app -w /app node:22 sh -c \
     "npm login && npm pkg set version=0.0.0 && npm publish --access public"
   git checkout package.json package-lock.json   # the 0.0.0 stays on npm only
   ```

2. **A trusted publisher on npmjs.com**: package settings, *Trusted
   Publisher*, GitHub Actions, with owner `blanpa`, repository
   `node-red-contrib-iolink-suite` and workflow filename `publish.yml`.
   No environment.

Provenance is only issued for public repositories, so the repository has to
be public before the first tag.
