...
Parse IO-Link **IODD** device description files and decode/encode IO-Link process data.

No Node-RED dependency, one runtime dependency (`fast-xml-parser`). If your process
data reaches you over PROFINET, Modbus, OPC UA or anything else, you can still use
this: hand it the raw bytes and the IODD, get engineering values back.

```js
const { parseIodd } = require('iodd-parser')
...
```js
const { IoddFinder } = require('iodd-parser/src/finder')
...
* **Unit codes** — `src/units.js`, generated by `npm run gen:units` from the OPC
...
* **Standard ISDU objects and EventCodes** — `src/standard.js`, from Tables B.8,
  D.1 and D.2 of the *IO-Link Interface and System Specification* V1.1.3.

## Tests

```bash
npm test          # unit tests against hand-written fixtures
