'use strict'
const { toBuffer } = require('./codec/bits')
/**
...
/**
 * Event Qualifier octet - Table A.14 of the specification.
 *
 *   bits 7..6  MODE      1 single shot, 2 disappears, 3 appears
 *   bits 5..4  TYPE      1 notification, 2 warning, 3 error
 *   bit  3     SOURCE    0 device (remote), 1 master (local)
 *   bits 2..0  INSTANCE  0 unknown, 4 application
 *
 * The raw octet is always reported alongside the resolved fields, so a device
 * that fills them in unconventionally can still be looked at.
 */
const EVENT_MODE = { 1: 'single shot', 2: 'disappears', 3: 'appears' }
const EVENT_TYPE = { 1: 'Notification', 2: 'Warning', 3: 'Error' }
const EVENT_SOURCE = { 0: 'device', 1: 'master' }
const EVENT_INSTANCE = { 0: 'unknown', 4: 'application' }

function decodeEventQualifier (octet) {
  const raw = Number(octet) & 0xff
  const instance = raw & 0x07
  return {
    qualifier: raw,
    mode: EVENT_MODE[(raw >> 6) & 0x03],
    type: EVENT_TYPE[(raw >> 4) & 0x03],
    source: EVENT_SOURCE[(raw >> 3) & 0x01],
    instance: EVENT_INSTANCE[instance] !== undefined ? EVENT_INSTANCE[instance] : instance
  }
}

/**
 * Decode DetailedDeviceStatus (ISDU index 37).
 *
 * The object is an array of three-octet entries: the Event Qualifier, then the
 * EventCode most significant octet first. Unused slots are all zero and are
 * dropped - a device declares a fixed number of them and fills as many as it
 * currently needs. A trailing partial entry is ignored rather than guessed at.
 *
 * @param {Buffer|Uint8Array|number[]|string} raw  the object as read from the device
 * @returns {object[]} one resolved event per occupied slot
 */
function decodeDetailedDeviceStatus (raw) {
  const buf = toBuffer(raw)
  const out = []
  for (let at = 0; at + 3 <= buf.length; at += 3) {
    const qualifier = buf[at]
    const code = (buf[at + 1] << 8) | buf[at + 2]
    if (qualifier === 0 && code === 0) continue
    out.push({ ...decodeEventQualifier(qualifier), ...lookupEvent(code, 'device') })
  }
  return out
}

/** The inverse of decodeEventQualifier(), for anything that has to build one. */
function encodeEventQualifier ({ mode = 'appears', type, source = 'device', instance = 'application' } = {}) {
  const code = (table, value, fallback) => {
    const hit = Object.entries(table).find(([, name]) => name === value)
    return hit ? Number(hit[0]) : fallback
  }
  const raw = (code(EVENT_MODE, mode, 3) << 6) |
    (code(EVENT_TYPE, type, 1) << 4) |
    (code(EVENT_SOURCE, source, 0) << 3) |
    (typeof instance === 'number' ? instance : code(EVENT_INSTANCE, instance, 0))
  return raw & 0xff
}

/**
 * Build a DetailedDeviceStatus object from a list of events.
 *
 * The counterpart of decodeDetailedDeviceStatus(): what a device would put in
 * index 37 while it is signalling these EventCodes. An event that does not say
 * what type it is takes the type the specification gives that code.
 *
 * @param {Array<number|string|object>} events  codes, or `{ code, mode, type }`
 * @param {object} [options]
 * @param {number} [options.slots]  pad to this many entries, as a device with
 *        room to spare reports it
 */
function encodeDetailedDeviceStatus (events = [], { slots = 0 } = {}) {
  const entries = events.map(entry => {
    const spec = typeof entry === 'object' && entry !== null ? entry : { code: entry }
    const code = typeof spec.code === 'string'
      ? Number.parseInt(spec.code, spec.code.toLowerCase().startsWith('0x') ? 16 : 10)
      : Number(spec.code)
    if (!Number.isInteger(code) || code < 0 || code > 0xffff) {
      throw new RangeError(`not an EventCode: ${JSON.stringify(spec.code)}`)
    }
    const known = lookupEvent(code, 'device')
    const qualifier = encodeEventQualifier({ ...spec, type: spec.type || (known && known.type) })
    return Buffer.from([qualifier, (code >> 8) & 0xff, code & 0xff])
  })
  while (entries.length < slots) entries.push(Buffer.alloc(3))
...
module.exports = {
  ...
  decodeEventQualifier,
  decodeDetailedDeviceStatus,
  encodeEventQualifier,
  encodeDetailedDeviceStatus
}
