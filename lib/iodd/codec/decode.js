...
function enumTextFor (item, rawValue) {
  ...
  // Compared as text: an IODD writes an enum's value as "1" or "true", and the
  // decoded value is a number or a boolean.
  const hit = item.values.find(v => String(v.value) === String(rawValue))
