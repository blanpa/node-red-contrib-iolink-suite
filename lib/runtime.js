 ...
 *
 * One cache serves every node on a master: three nodes reading port 1 ask the
 * port once, not three times, and each may say how old an answer it will take.
 * A shorter wish refreshes the entry for everyone, so the node that needs to
 * notice a swapped device quickly is not held back by one that does not.
 *
 * A port the master could not be asked about is not remembered. The adapters
 * report an unreachable master as an entry carrying `error`, and caching that
 * would keep the outage alive for a full TTL after the master came back.
 */
class PortIdentityCache {
  ...
  async get (adapter, port, { force = false, ttl } = {}) {
    const now = Date.now()
    const maxAge = ttl === undefined ? this.ttl : ttl
    const hit = this.entries.get(port)
    if (!force && hit && now - hit.at < maxAge) return hit.value
    const [status] = await adapter.scanPorts([port])
    if (status && !status.error) this.entries.set(port, { at: now, value: status })
    else this.entries.delete(port)
    return status
  }
...
/**
 * The identity cache shared by every node on a master.
 *
 * It hangs off the master node so it lives as long as the connection does and
 * is cleared with it; a master that has none yet (a stub in a test, an older
 * config node) gets one on first use.
 */
function sharedIdentityCache (master) {
  if (!master.identity) master.identity = new PortIdentityCache()
...
async function resolveDevice (master, port, options = {}) {
  ...
  if (!vendorId || !deviceId) {
    status = await options.identityCache.get(master.adapter, port,
      { force: options.force, ttl: options.identityTtl })
    if (status && status.error) {
      // The master could not be asked, which says nothing about the port. An
      // outage reported as "no device" sent people to check a cable when the
      // whole rack had gone quiet.
      throw Object.assign(
        new Error(`port ${port} could not be asked: ${status.error}`),
        { code: 'IOLINK_MASTER_UNREACHABLE', port })
    }
    if (!status || !status.vendorId || !status.deviceId) {
      // Two different situations, and telling them apart is the difference
      // between looking at the wiring and looking at the configuration: the
      // port is empty, or something is plugged in and this master cannot say
      // what. The second used to be reported as the first, which sent people
      // hunting for a device that was there all along.
      if (status && status.connected) {
        throw Object.assign(
          new Error(
            `port ${port} has a device, but this master does not report which: ` +
            'set the vendor and device id on the node, or configure the ' +
            "profile's readVendorId and readDeviceId paths"),
          { code: 'IOLINK_NO_IDENTITY', port })
      }
...
/**
 * The `device` block every node puts on its message.
 *
 * The product name comes from the master when it reports one: an IODD often
 * describes a whole family, and its first variant is not necessarily the one
 * on the port. The IODD fills in for a master that does not say.
 */
function describeDevice (device, status, port) {
  const identity = device.identity
  return {
    vendor: identity.vendorName,
    vendorId: identity.vendorId,
    deviceId: identity.deviceId,
    product: (status && status.productName) ||
      (identity.variants[0] || {}).productId || identity.deviceName,
 ...
 *
 * The error handed to a Catch node carries its code up front, so a flow can
 * branch on it whichever node it came from.
 */
function fail (node, msg, error, done) {
  node.status({ fill: 'red', shape: 'ring', text: shorten(error.message) })
  const reported = withCode(error)
  if (done) done(reported)
  else node.error(reported.message, msg)
}

/**
 * The same error with its code in front of the message.
 *
 * A new error rather than a rewritten one. The same failure is handed to more
 * than one caller - the IODD store remembers a lookup that failed and gives
 * every read inside its retry window the very same object - and rewriting the
 * message in place stacked the code onto it once per read.
 */
function withCode (error) {
  if (!error.code || String(error.message).startsWith(`${error.code}: `)) return error
  const out = new Error(`${error.code}: ${error.message}`)
...
/**
 * Resolve the ports a node should work on, as a list.
 *
 * The same field that holds one port holds several - "1,3", or an array out of
 * a message - so reading one sensor and reading a rack is one node with a
 * different field rather than two nodes. One port comes back as a list of one;
 * whether that still means one flat payload is the caller's decision, and for
 * the read node it does, because a flow that reads a single port must not
 * change shape because this became possible.
 *
 * A port named twice is read once. Two identical keys in the output would be
 * one key, so the second read would be work done to be thrown away.
 */
function resolvePorts (RED, node, msg, config) {
  const configured = config.port
  const type = config.portType || 'num'
  ...
  if (!ports || !ports.length) {
    const from = type === 'num' || type === 'str'
      ...
      : `${type}.${configured}, which held ${JSON.stringify(raw)},`
    throw Object.assign(
      new Error(`${from} is not a port number, or a list of them`),
...
/**
 * A hex string as the codec wants it: even length, digits only, lower case.
 *
 * `Buffer.from(text, 'hex')` stops quietly at the first thing it cannot read,
 * so a master answering "ERR" or a flow writing "0x1" would decode as nothing
 * or as half a value. Separators people put between octets are allowed.
 */
function parseHex (value, what = 'value') {
  const text = typeof value === 'string' || typeof value === 'number'
    ? String(value).trim().replace(/^0x/i, '').replace(/[\s:_-]/g, '')
    : null
  if (text === null || !/^[0-9a-fA-F]*$/.test(text)) {
    throw Object.assign(
      new Error(`${what} is not hex: ${JSON.stringify(value)}`), { code: 'IOLINK_BAD_HEX' })
  }
  if (text.length % 2) {
    throw Object.assign(
      new Error(`${what} has an odd number of hex digits: ${JSON.stringify(value)}`),
      { code: 'IOLINK_BAD_HEX' })
  }
