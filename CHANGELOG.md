...
- The README shows the architecture, the IODD lookup, one read from message
  to values, and the port states of `iolink event` as diagrams.
- The release workflow publishes through npm's trusted publishing (OIDC)
  rather than with a token in a secret. The one-time setup is in the README.
